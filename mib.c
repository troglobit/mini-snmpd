...
static int encode_oid(data_t *data, const oid_t *oid)
{
	...
	if (len > 0xFFFF) {
		logit(LOG_ERR, "Failed encoding '%s': OID overflow\n", oid_ntoa(oid));
		return -1;
	}
...
static int mib_build_ip_entry(const oid_t *prefix, int type, const void *arg)
{
	...
	if (g_mib_length >= MAX_NR_VALUES) {
		logit(LOG_ERR, "%s '%s': table overflow\n", msg, oid_ntoa(prefix));
		return -1;
	}
	...
	if (ret) {
		logit(LOG_ERR, "%s '%s': unsupported type %d\n", msg,
	...
	if (ret) {
		if (ret == 1)
			...
			logit(LOG_ERR, "%s '%s': invalid default value\n", msg2, oid_ntoa(&value->oid));
...
static value_t *mib_alloc_entry(const oid_t *prefix, int column, int row, int type)
{
	...
	/* Create a new entry in the MIB table */
	if (g_mib_length >= MAX_NR_VALUES) {
		logit(LOG_ERR, "%s '%s.%d.%d': table overflow\n", msg, oid_ntoa(prefix), column, row);
		return NULL;
	}
	...
	if (oid_build(&value->oid, prefix, column, row)) {
		logit(LOG_ERR, "%s '%s.%d.%d': oid overflow\n", msg, oid_ntoa(prefix), column, row);
		return NULL;
	}
	...
	ret += data_alloc(&value->data, type);
	if (ret) {
		logit(LOG_ERR, "%s '%s.%d.%d': unsupported type %d\n", msg,
...
static int mib_data_set(const oid_t *oid, data_t *data, int column, int row, int type, const void *arg)
{
	...
	if (ret) {
		if (ret == 1)
			...
			logit(LOG_ERR, "%s '%s.%d.%d': invalid default value\n", msg, oid_ntoa(oid), column, row);
...
static value_t *mib_value_find(const oid_t *prefix, int column, int row, size_t *pos)
{
	...
	if (!value)
		logit(LOG_ERR, "%s '%s.%d.%d': OID not found\n", msg, oid_ntoa(prefix), column, row);
